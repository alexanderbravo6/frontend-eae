...
function EliminarEvaluacionButton({ row }) {

    const { mutate } = useSWRConfig();
    const [isLoading, setIsLoading] = useState(false)
    const { eliminarMatricula } = useSeguimientoService()
    const { query, pagination } = useSeguimientoEvaluacion()
    const { ValidarPermisos } = useUtils()
    if (!ValidarPermisos('SEGEVA', 'ELI')) return null
        ...
        try {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await eliminarMatricula(row.id)
    ...
    return (
        <>
            {
                row.estado === 0 && <ButtonDelete action={handleEliminar} isLoading={isLoading} />
            }
        </>
    )
}

export default EliminarEvaluacionButton