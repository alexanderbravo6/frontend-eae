...
import ParticipantePruebaCard from '../Cards/ParticipantePruebaCard';

function ParticipantePruebaTab({ idCiclo, row }) {
    const { data: session } = useSession();

    const { FetchParticipantePorPrueba } = useResultadoInstitucionalService();
    const resultados = FetchParticipantePorPrueba(session?.user?.anio, idCiclo, row.id);
                            ...
                            <section className=' flex flex-wrap gap-5'>
                                {
                                    item.pruebas.length > 0 ?
                                        item.pruebas.map((prueba, i) => (
                                            <ParticipantePruebaCard key={i} data={prueba} />
...
export default ParticipantePruebaTab